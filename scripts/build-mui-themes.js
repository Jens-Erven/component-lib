 ...
 * Uses @jens_erven/design-tokens-brio npm package
 */
const generateMuiThemes = async () => {
  try {
    ...
    // Get theme names from npm package
    const themeNames = ["amsterdam", "barcelona", "berlin", "lisbon", "london"];
...
function generateThemeFileContent(themeNames) {
  ...
  // Generate imports for light themes from npm package
  const lightImports = themeNames
    .map(
      (theme) =>
        `import * as ${theme}Light from '@jens_erven/design-tokens-brio/tokens/${theme}/light';`
    )
    .join("\n");

  // Generate imports for dark themes from npm package
  const darkImports = themeNames
    .map(
      (theme) =>
        `import * as ${theme}Dark from '@jens_erven/design-tokens-brio/tokens/${theme}/dark';`
    )
    .join("\n");
 ...
 * This file is automatically generated by scripts/build-mui-themes.js
 * To update, ensure @jens_erven/design-tokens-brio is up to date and run: npm run build:mui-themes
 * 
 * @module themes
 * 
 * USAGE EXAMPLES:
...
${themeNames
  .map(
    (t) =>
      ` * - allThemes['${capitalizeFirst(
        t
      )} Light'], allThemes['${capitalizeFirst(t)} Dark']`
  )
  .join("\n")}
 * 
 * TypeScript Types:
...
// Import light themes from @jens_erven/design-tokens-brio
${lightImports}

// Import dark themes from @jens_erven/design-tokens-brio
${darkImports}
