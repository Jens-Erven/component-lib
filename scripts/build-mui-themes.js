 ...
 * Uses @jens_erven/design-tokens npm package
 */
...
function generateThemeFileContent(themeNames) {
  ...
  const lightImports = themeNames
    .map(
      (theme) =>
        `import * as ${theme}Light from '@jens_erven/design-tokens/tokens/${theme}/light';`
    )
    .join("\n");
  ...
  const darkImports = themeNames
    .map(
      (theme) =>
        `import * as ${theme}Dark from '@jens_erven/design-tokens/tokens/${theme}/dark';`
    )
    .join("\n");
 ...
 * To update, ensure @jens_erven/design-tokens is up to date and run: npm run build:mui-themes
 * 
 * @module themes
 * 
 * USAGE EXAMPLES:
...
// Import light themes from @jens_erven/design-tokens
${lightImports}

// Import dark themes from @jens_erven/design-tokens
${darkImports}
